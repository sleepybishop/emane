...
pub mod transponder;
pub mod transponder_configuration;
pub mod transponder_noprotocol;
pub mod transponder_tdmaprotocol;
...
pub static mut FW_SERVICE: Option<FfiFrameworkService> = None;

extern "C" fn init(id: u16, fw_service: *const FfiFrameworkService) -> *mut c_void {
    println!("bentpipemaclayer init {}", id);
    unsafe {
...
pub extern "C" fn emane_plugin_create() -> *const PluginApi {
    unsafe {
        if API.is_none() {
            API = Some(PluginApi {
                name: CString::new("bentpipemaclayer").unwrap().into_raw(),
                plugin_type: 1, // MAC
                init,
                configure,
                start,

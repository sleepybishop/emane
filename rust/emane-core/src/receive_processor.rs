use crate::gain_manager::emane_rs_gain_manager_determine_gain;
use crate::spectrum_monitor::{FfiFrequencySegment, SpectrumMonitor};
...
extern "C" {
    ...
    fn emane_c_common_phy_header_get_frequency_segments_ptr(
        hdr_ptr: *const c_void,
        group_idx: usize,
    ) -> *const c_void;
    fn emane_c_frequency_segments_size(segs_ptr: *const c_void) -> usize;
    fn emane_c_frequency_segments_get(
        segs_ptr: *const c_void,
        seg_idx: usize,
        freq: *mut u64,
        power: *mut f64,
        has_power: *mut bool,
    ...
    fn emane_c_common_phy_header_get_transmit_antenna(
        hdr_ptr: *const c_void,
        idx: usize,
        freq_group_idx: *mut usize,
        ant_idx: *mut u16,
        bw: *mut u64,
        mask_idx: *mut u16,
    );
    ...
    fn emane_c_common_phy_header_get_transmitter(
        hdr_ptr: *const c_void,
        idx: usize,
        nem_id: *mut u16,
        power_dbm: *mut f64,
    ...
    fn emane_c_common_phy_header_get_optional_filter_data(
        hdr_ptr: *const c_void,
    ...
    fn emane_c_antenna_interferences_get(
        ptr: *const c_void,
        idx: usize,
        freq_group_idx: *mut usize,
        out_powers: *mut *const f64,
        out_powers_len: *mut usize,
    );
    ...
    fn emane_c_propagation_model_compute(
        algo_ptr: *mut c_void,
        src: u16,
        loc_info_ptr: *const c_void,
        segments_ptr: *const c_void,
    ) -> EMANE_PathlossResult;
    fn emane_c_propagation_model_free_result(res: *mut EMANE_PathlossResult);
    fn emane_c_fading_store_compute_with_model(
    ...
    fn emane_c_receive_processor_add_antenna_receive_info(
        ...
        freq_segments_ptr: *const c_void,
    );
...
pub extern "C" fn emane_rs_receive_processor_create(
    ...
    let rp = Box::new(ReceiveProcessorImpl {
        ...
        spectrum_monitor,
        propagation_model,
        fading_algorithm_store,
        populate_receive_power_map,
        populate_observed_power_map,
        doppler_shift,
    });
...
pub extern "C" fn emane_rs_receive_processor_destroy(ptr: *mut c_void) {
    if !ptr.is_null() {
        unsafe {
            drop(Box::from_raw(ptr as *mut ReceiveProcessorImpl));
        }
    }
}
...
pub extern "C" fn emane_rs_receive_processor_process(
    ...
    let num_freq_groups =
        unsafe { emane_c_common_phy_header_get_frequency_groups_size(common_phy_header) };
    let num_tx_antennas =
        unsafe { emane_c_common_phy_header_get_transmit_antennas_size(common_phy_header) };
    let num_transmitters =
        unsafe { emane_c_common_phy_header_get_transmitters_size(common_phy_header) };
    ...
    for ant_i in 0..num_tx_antennas {
        ...
        unsafe {
            emane_c_common_phy_header_get_transmit_antenna(
                common_phy_header,
                ant_i,
                &mut group_idx,
                &mut ant_idx,
        ...
        if group_idx >= num_freq_groups {
            unsafe {
                emane_c_receive_processor_set_status(result_ptr, 1);
            } // DROP_CODE_ANTENNA_FREQ_INDEX
            return;
        }

        let segments_ptr = unsafe {
            emane_c_common_phy_header_get_frequency_segments_ptr(common_phy_header, group_idx)
        };
        ...
        for tx_i in 0..num_transmitters {
            ...
            unsafe {
                emane_c_common_phy_header_get_transmitter(
                    common_phy_header,
                    tx_i,
                    &mut tx_nem_id,
                    &mut tx_power_dbm,
            ...
            let loc_ptr = unsafe {
                emane_c_location_infos_get(location_infos, i_transmitter_index, &mut loc_valid)
            };
            let mut fading_valid = false;
            let mut fading_model = 0;
            let fading_selection = unsafe {
                emane_c_fading_infos_get(
                    fading_infos,
                    i_transmitter_index,
                    &mut fading_valid,
                    &mut fading_model,
            ...
            let mut pathloss_info = unsafe {
                emane_c_propagation_model_compute(
                    rp.propagation_model,
                    tx_nem_id,
                    loc_ptr,
                    segments_ptr,
                )
                ...
                && pathloss_info.count == num_segments
            {
                ...
                if gain_result.status == 0 {
                    // SUCCESS
                    unsafe {
                        emane_c_receive_processor_set_gain_cache_hit(
                            result_ptr,
                            gain_result.is_cache,
                        );
                    }

                    let pathlosses = unsafe {
                        slice::from_raw_parts(pathloss_info.pathlosses, pathloss_info.count)
                    };
                    ...
                    for s_idx in 0..num_segments {
                        ...
                        unsafe {
                            emane_c_frequency_segments_get(
                                segments_ptr,
                                s_idx,
                                &mut seg_freq,
                                &mut seg_power,
                        ...
                        let tx_pwr = if seg_has_power {
                            seg_power
                        } else {
                            tx_power_dbm
                        };
                        let power_dbm = tx_pwr + gain_result.remote_gain + gain_result.local_gain
                            - pathlosses[s_idx];
                        ...
                        if fading_valid {
                            if fading_model == 0 {
                                ...
                                if loc_valid {
                                    let dist =
                                        unsafe { emane_c_location_info_get_distance(loc_ptr) };
                                    rx_pwr_mw += unsafe {
                                        emane_c_fading_store_compute_with_model(
                                            rp.fading_algorithm_store,
                                            fading_model,
                                            power_dbm,
                                            dist,
                                            fading_selection,
                                    ...
                                    unsafe {
                                        emane_c_propagation_model_free_result(&mut pathloss_info);
                                    }
                                    unsafe {
                                        emane_c_receive_processor_set_status(result_ptr, 2);
                                    } // FADINGMANAGER_LOCATION
                                    return;
                                }
                            }
                        } else {
                            ...
                            unsafe {
                                emane_c_receive_processor_set_status(result_ptr, 4);
                            } // FADINGMANAGER_SELECTION
                            return;
                        }
                        ...
                        if rp.doppler_shift {
                            ...
                            unsafe {
                                emane_c_receive_processor_add_doppler_shift(
                                    result_ptr,
                                    seg_freq,
                                    d_doppler_hz,
                                );
                        ...
                        if rp.populate_receive_power_map {
                            unsafe {
                                emane_c_receive_processor_add_receive_power(
                                    ...
                                    ant_idx,
                                    seg_freq,
                                    milliwatt_to_db(rx_pwr_mw),
                                    gain_result.remote_gain,
                                    gain_result.local_gain,
                                    tx_pwr,
                                    pathlosses[s_idx],
                                    d_doppler_hz,
                                );
                    ...
                    unsafe {
                        emane_c_receive_processor_set_status(result_ptr, st);
                    }
                    return;
                }
            } else {
                ...
                unsafe {
                    emane_c_receive_processor_set_status(result_ptr, 9);
                } // PROPAGATIONMODEL
                return;
            }
            unsafe {
        ...
        let opt_filter_ptr = unsafe {
            emane_c_common_phy_header_get_optional_filter_data(
                common_phy_header,
                &mut opt_filter_has,
                &mut opt_filter_len,
            )
        ...
        let (sot, prop_delay, span, res_segments, treat_in_band, sensitivity) = monitor.update(
            ...
            if opt_filter_has { opt_filter_len } else { 0 },
        );

        if true {
            // Since it returns a tuple directly, not Option
            if treat_in_band {
                if !res_segments.is_empty() {
                    unsafe {
                        emane_c_receive_processor_set_mimo(
                            result_ptr,
                            sot as u64,
                            prop_delay as u64,
                        );
                    ...
                    unsafe {
                        emane_c_receive_processor_add_antenna_receive_info(
                            result_ptr,
                            rp.rx_antenna_index,
                            ant_idx,
                            span as u64,
                            milliwatt_to_db(sensitivity),
                            segments_ptr,
                        );
                    }
    ...
    unsafe {
        emane_c_receive_processor_set_status(result_ptr, final_status);
    }
}
...
pub extern "C" fn emane_rs_receive_processor_process_self_interference(
    ...
    for ant_i in 0..num_interferences {
        ...
        unsafe {
            emane_c_antenna_interferences_get(
                antenna_interferences,
                ant_i,
                &mut group_idx,
                &mut powers_ptr,
                &mut powers_len,
        ...
        let segments_ptr =
            unsafe { emane_c_frequency_groups_get_segments_ptr(frequency_groups, group_idx) };
        let num_segments = unsafe { emane_c_frequency_segments_size(segments_ptr) };
        ...
        if powers_len == 1 {
            for i in 0..num_segments {
                rx_power_segments_milliwatt[i] = supplied_powers[0];
            }
        } else if num_segments == powers_len {
            for i in 0..num_segments {
                rx_power_segments_milliwatt[i] = supplied_powers[i];
            }
        } else {

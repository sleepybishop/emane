    ...
    if !ptr.is_null() {
        ...
        for (_, mut inner_map) in mgr.store.drain() {
            for (_, info) in inner_map.drain() {
                unsafe { emane_rs_ffi_free_antenna_info(info); }
            }
        }
        for (_, pt) in mgr.default_pointing.drain() {
            unsafe { emane_rs_ffi_free_pointing(pt); }
        }
    }
}

#[no_mangle]
pub extern "C" fn emane_rs_antenna_manager_insert_antenna_info(ptr: *mut c_void, nem_id: u16, idx: u16, info: *mut c_void) {
    ...
    if let Some(old) = inner.insert(idx, info) {
        if old != info {
            unsafe { emane_rs_ffi_free_antenna_info(old); }
        }
    }
}

#[no_mangle]
pub extern "C" fn emane_rs_antenna_manager_get_antenna_info(ptr: *mut c_void, nem_id: u16, idx: u16) -> *mut c_void {
    let mgr = unsafe { &mut *(ptr as *mut AntennaManagerImpl) };
...
#[no_mangle]
pub extern "C" fn emane_rs_antenna_manager_remove_antenna_info(ptr: *mut c_void, nem_id: u16, idx: u16) {
    let mgr = unsafe { &mut *(ptr as *mut AntennaManagerImpl) };
...
#[no_mangle]
pub extern "C" fn emane_rs_antenna_manager_insert_default_pointing(ptr: *mut c_void, nem_id: u16, pointing: *mut c_void) {
    let mgr = unsafe { &mut *(ptr as *mut AntennaManagerImpl) };
    if let Some(old) = mgr.default_pointing.insert(nem_id, pointing) {
        if old != pointing {
            unsafe { emane_rs_ffi_free_pointing(old); }
        }
    }
}

#[no_mangle]
pub extern "C" fn emane_rs_antenna_manager_get_default_pointing(ptr: *mut c_void, nem_id: u16) -> *mut c_void {
    let mgr = unsafe { &mut *(ptr as *mut AntennaManagerImpl) };
    mgr.default_pointing.get(&nem_id).copied().unwrap_or(std::ptr::null_mut())
}

...
extern "C" {
    ...
    fn emane_c_antenna_manager_get_info(
        ...
        nem_id: u16,
        index: u16,
        out_is_omni: *mut bool,
        out_fixed_gain: *mut f64,
        out_has_pointing: *mut bool,
        out_profile_id: *mut u16,
        out_pointing_azimuth: *mut f64,
        out_pointing_elevation: *mut f64,
        out_seq: *mut u64,
    ) -> bool;
...
fn get_antenna_info(am_ptr: *mut c_void, nem_id: u16, index: u16) -> Option<AntennaInfo> {
    ...
    let ok = unsafe {
        emane_c_antenna_manager_get_info(
            ...
            &mut has_pointing,
            &mut profile_id,
            &mut pointing_azimuth,
            &mut pointing_elevation,
            &mut seq,
        )
    };
    if ok {
...
#[no_mangle]
pub extern "C" fn emane_rs_gain_manager_create(
    nem_id: u16,
    rx_antenna_index: u16,
    am_ptr: *mut c_void,
) -> *mut c_void {
...
pub extern "C" fn emane_rs_gain_manager_determine_gain(
    ...
    if !remote_ant_info.is_omni {
        ...
        if remote_ant_info.has_pointing {
            if let Some((p_pattern, p_blockage, p_north, p_east, p_up)) =
                crate::antenna::get_manager().get_profile_info(remote_ant_info.profile_id)
            {
                let local_n = if let Some(p) =
                    crate::antenna::get_manager().get_profile_info(local_ant_info.profile_id)
                {
                    p.2
                } else {
                    0.0
                };
                let local_e = if let Some(p) =
                    crate::antenna::get_manager().get_profile_info(local_ant_info.profile_id)
                {
                    p.3
                } else {
                ...
                let direction = unsafe {
                    emane_c_utils_calculate_direction_by_neu(
                        location_pair_info,
                        p_north,
                        p_east,
                        p_up,
                ...
                let lookup = unsafe {
                    emane_c_utils_calculate_lookup_angles(
                        direction.azimuth,
                        remote_ant_info.pointing_azimuth,
                        direction.elevation,
                        remote_ant_info.pointing_elevation,
                    )
                };

                let tx_gain = unsafe {
                    emane_rs_antenna_pattern_get_gain(
                        p_pattern as *const c_void,
                        lookup.azimuth.round() as i16,
                        lookup.elevation.round() as i16,
                    )
                };
                ...
                let tx_blockage = if !p_blockage.is_null() {
                    unsafe {
                        emane_rs_antenna_pattern_get_gain(
                            p_blockage as *const c_void,
                            direction.azimuth.round() as i16,
                            direction.elevation.round() as i16,
                        )
                    }
                } else {
                    0.0
        ...
        if local_ant_info.has_pointing {
                ...
                crate::antenna::get_manager().get_profile_info(local_ant_info.profile_id)
            {
                ...
                let lookup = unsafe {
                    emane_c_utils_calculate_lookup_angles(
                        direction.azimuth,
                        local_ant_info.pointing_azimuth,
                        direction.elevation,
                        local_ant_info.pointing_elevation,
                    )
                };
    ...
    if is_loc_valid && dist > 10.0 {
        ...
        let ok = unsafe {
            emane_c_utils_check_horizon(local_alt + local_u, remote_alt + remote_u, dist)
        };
        if !ok {
            return EmaneGainResult {
                remote_gain: 0.0,

...
impl AntennaPattern {
    pub fn new(uri: &str, sub_root_name: &str, missing_value: f64) -> Result<Self, String> {
        let content = std::fs::read_to_string(uri).map_err(|e| format!("Unable to read {}: {}", uri, e))?;
        let doc = Document::parse(&content).map_err(|e| format!("Validation failure {}: {}", uri, e))?;
        ...
        for node in root.children().filter(|n| n.is_element() && n.tag_name().name() == sub_root_name) {
            let mut elevation_range_max: i16 = 0;
            let mut first_elevation = true;

            for elevation_node in node.children().filter(|n| n.is_element() && n.tag_name().name() == "elevation") {
                let min_str = elevation_node.attribute("min").ok_or_else(|| format!("Bad entry {}: elevation missing min", uri))?;
                let elevation_min: i16 = min_str.parse().map_err(|_| format!("Bad entry {}: elevation min parse", uri))?;
                
                let max_str = elevation_node.attribute("max").ok_or_else(|| format!("Bad entry {}: elevation missing max", uri))?;
                let elevation_max: i16 = max_str.parse().map_err(|_| format!("Bad entry {}: elevation max parse", uri))?;

                if elevation_min > elevation_max {
                    return Err(format!("Bad entry {}: elevation [{},{}] min > max", uri, elevation_min, elevation_max));
                }

                if !first_elevation && elevation_range_max >= elevation_min && !pattern.elevation_bearing_gain.is_empty() {
                ...
                for bearing_node in elevation_node.children().filter(|n| n.is_element() && n.tag_name().name() == "bearing") {
                    let b_min_str = bearing_node.attribute("min").ok_or_else(|| format!("Bad entry {}: bearing missing min", uri))?;
                    let bearing_min: i16 = b_min_str.parse().map_err(|_| format!("Bad entry {}: bearing min parse", uri))?;

                    let b_max_str = bearing_node.attribute("max").ok_or_else(|| format!("Bad entry {}: bearing missing max", uri))?;
                    let bearing_max: i16 = b_max_str.parse().map_err(|_| format!("Bad entry {}: bearing max parse", uri))?;

                    if bearing_min > bearing_max {
                        return Err(format!("Bad entry {}: elevation [{},{}] bearing [{},{}] min > max", uri, elevation_min, elevation_max, bearing_min, bearing_max));
                    }

                    if !first_bearing && bearing_range_max >= bearing_min && !bearing_gain_map.is_empty() {
                    ...
                    for gain_node in bearing_node.children().filter(|n| n.is_element() && n.tag_name().name() == "gain") {
                        let gain_str = gain_node.attribute("value").ok_or_else(|| format!("Bad entry {}: gain missing value", uri))?;
                        let gain: f64 = gain_str.parse().map_err(|_| format!("Bad entry {}: gain value parse", uri))?;
                ...
                if !pattern.elevation_bearing_gain.contains_key(&(elevation_min - 1)) {
                    pattern.elevation_bearing_gain.insert(elevation_min - 1, None);
                }
                pattern.elevation_bearing_gain.insert(elevation_max, Some(bearing_gain_map));
            }
        }
...
impl AntennaProfileManifest {
    ...
    fn get_or_load_pattern(&mut self, uri: &str, sub_root: &str, missing: f64) -> Result<usize, String> {
    ...
    pub fn load(&mut self, uri: &str) -> Result<(), String> {
        ...
        for node in root.children().filter(|n| n.is_element() && n.tag_name().name() == "profile") {
            let id_str = node.attribute("id").ok_or_else(|| format!("profile missing id"))?;
            let id: u16 = id_str.parse().map_err(|_| format!("profile id parse"))?;

            let antenna_uri = node.attribute("antennapatternuri").ok_or_else(|| format!("profile missing antennapatternuri"))?;
            ...
            for child in node.children().filter(|n| n.is_element() && n.tag_name().name() == "placement") {
                north = child.attribute("north").unwrap_or("0").parse().unwrap_or(0.0);
                east = child.attribute("east").unwrap_or("0").parse().unwrap_or(0.0);
                up = child.attribute("up").unwrap_or("0").parse().unwrap_or(0.0);
            }

            let ant_idx = self.get_or_load_pattern(antenna_uri, "antennapattern", -300.0 /* DBM_MIN */)?;
            ...
            self.profiles.insert(id, (ant_idx, blk_idx, north, east, up));
        }

        Ok(())
    }

    pub fn get_profile_info(&self, id: u16) -> Option<(*const AntennaPattern, *const AntennaPattern, f64, f64, f64)> {
        if let Some(&(ant_idx, blk_idx, north, east, up)) = self.profiles.get(&id) {
            let ant_ptr = &self.patterns[ant_idx] as *const AntennaPattern;
            let blk_ptr = if blk_idx == usize::MAX { std::ptr::null() } else { &self.patterns[blk_idx] as *const AntennaPattern };
...
#[no_mangle]
pub extern "C" fn emane_rs_antenna_profile_load(uri: *const c_char, error_buf: *mut c_char, error_buf_len: usize) {
...
pub extern "C" fn emane_rs_antenna_pattern_get_gain(
    ...
    unsafe {
        (*pattern).get_gain(bearing, elevation)
    }
}

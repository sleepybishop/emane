...
#[no_mangle]
pub extern "C" fn emane_rs_eel_generator_new(c_generator: *mut c_void, callbacks: EelCallbacks) -> *mut EelGenerator {
...
#[no_mangle]
pub extern "C" fn emane_rs_eel_generator_add_input_file(ptr: *mut EelGenerator, filename: *const c_char) {
...
pub extern "C" fn emane_rs_eel_generator_add_loader(
    ...
    let gen = unsafe { &mut *ptr };
    let loader = unsafe { CStr::from_ptr(loader_str) }.to_string_lossy().into_owned();
    
    // format: EventTypes:LibraryName[:PublishMode]
    let parts: Vec<&str> = loader.split(':').collect();
    if parts.len() < 2 {
        let err = format!("EEL::Generator: Bad configuration 'loader' format {}", loader);
        unsafe { *error_out = CString::new(err).unwrap().into_raw(); }
        return false;
    }
    ...
    match EelLoaderPluginFactory::construct(&lib_filename) {
        Ok(factory) => {
            ...
            for ev_type in event_types.split(',') {
                gen.plugins.insert(ev_type.to_string(), (plugin_ptr as usize, publish_mode));
            }
            true
        },
        Err(e) => {
...
#[no_mangle]
pub extern "C" fn emane_rs_eel_generator_get_plugin(ptr: *const EelGenerator, event_type: *const c_char) -> *mut c_void {
    let gen = unsafe { &*ptr };
...
fn generate(c_generator_usize: usize, input_files: Vec<String>, shared: Arc<SharedState>, callbacks: EelCallbacks, plugins_map: HashMap<String, (usize, u8)>) {
    ...
    for filename in input_files {
        ...
        for line_res in reader.lines() {
            ...
            if let Ok(Some((f_event_time, s_module_id, s_event_type, input_args))) = EelInputParser::parse(&line) {
                if f_event_time == 0.0 && s_module_id.is_empty() && s_event_type.is_empty() && input_args.is_empty() {
                    continue;
                }
                ...
                if f_event_time != f_current_time {
                    if !wait_and_send_events(c_generator, test_start_time, f_current_time, &shared, &callbacks, &unique_plugins) {
                        return;
                    }
                ...
                if let Some(&(plugin_ptr_usize, _)) = plugins_map.get(&s_event_type) {
                    ...
                    unsafe {
                        (callbacks.plugin_load)(plugin_ptr_usize as *mut c_void, c_module_type.as_ptr(), u16_module_id, c_event_type.as_ptr(), c_args_ptrs.as_ptr(), c_args_ptrs.len());
                    }
                }
            }
        }
    ...
    wait_and_send_events(c_generator, test_start_time, f_current_time, &shared, &callbacks, &unique_plugins);
}

fn wait_and_send_events(c_generator: *mut c_void, test_start_time: Instant, f_current_time: f32, shared: &Arc<SharedState>, callbacks: &EelCallbacks, plugins: &Vec<(usize, u8)>) -> bool {
    ...
    for &(p_usize, publish_mode) in plugins {
...
pub extern "C" fn emane_rs_eel_generator_start(ptr: *mut EelGenerator) {
    ...
    let c_gen_sync = c_generator as usize; gen.thread = Some(thread::spawn(move || { let c_generator = c_gen_sync as *mut c_void;

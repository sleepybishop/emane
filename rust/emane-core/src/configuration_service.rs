use crate::regex::{emane_rs_regex_compile, emane_rs_regex_match};
...
extern "C" {
    ...
    fn emane_c_config_process_configuration(
...
#[no_mangle]
pub extern "C" fn emane_rs_config_register_running_state_mutable(
    build_id: u16,
...
pub extern "C" fn emane_rs_config_register_numeric_any(
    ...
    any_type: i32,
    properties: u64,
    values_ptr: *const FfiAny,
    values_len: usize,
    s_usage: *const c_char,
    min_ptr: *const FfiAny,
    max_ptr: *const FfiAny,
    min_occurs: usize,
    max_occurs: usize,
    s_regex: *const c_char,
    err_buf: *mut c_char,
    err_len: usize,
) {
    let mut s = get_config_service().lock().unwrap();
    ...
    if name.chars().any(|c| !c.is_alphanumeric() && c != '.') {
        write_error(
            &format!("Invalid character in configuration name: {}", name),
            err_buf,
            err_len,
        );
        return;
    ...
    if store.contains_key(&name) {
        write_error(
            &format!(
                "Duplicate configuration name registration detected: {}",
                name
            ),
            err_buf,
            err_len,
    ...
    if !regex_pattern.is_empty() {
        ...
        if regex_ptr.is_null() {
            ...
            write_error(
                &format!(
                    "Bad regex pattern defined for {}: {} {}",
                    name, regex_pattern, err_msg
                ),
                err_buf,
                err_len,
            );
...
pub extern "C" fn emane_rs_config_register_non_numeric_any(
    ...
    if !regex_pattern.is_empty() {
        ...
        if regex_ptr.is_null() {
            let err_msg = unsafe { CStr::from_ptr(regex_err.as_ptr()).to_string_lossy() };
            write_error(
                &format!("Invalid regex for {}: {}", name, err_msg),
                err_buf,
                err_len,
            );
            return;
...
pub extern "C" fn emane_rs_config_get_manifest(build_id: u16) -> FfiConfigManifest {
    ...
    for info in store.values() {
        let c_name = std::ffi::CString::new(info.name.clone())
            .unwrap()
            .into_raw();
        let c_usage = std::ffi::CString::new(info.usage.clone())
            .unwrap()
            .into_raw();
        let c_regex = if info.regex_pattern.is_empty() {
            std::ptr::null()
        } else {
            std::ffi::CString::new(info.regex_pattern.clone())
                .unwrap()
                .into_raw()
        };
        ...
        for v in &info.values {
            let s_val = if v.s_value.is_empty() {
                ...
                std::ffi::CString::new(v.s_value.clone())
                    .unwrap()
                    .into_raw()
            };
        ...
        let min_s_val = if info.min_value.s_value.is_empty() {
            std::ptr::null()
        } else {
            std::ffi::CString::new(info.min_value.s_value.clone())
                .unwrap()
                .into_raw()
        };
        let max_s_val = if info.max_value.s_value.is_empty() {
            std::ptr::null()
        } else {
            std::ffi::CString::new(info.max_value.s_value.clone())
                .unwrap()
                .into_raw()
        };

        let ffi_min = FfiAny {
            any_type: info.min_value.any_type,
            i64_value: info.min_value.i64_value,
            u64_value: info.min_value.u64_value,
            d_value: info.min_value.d_value,
            s_value: min_s_val,
        };
        let ffi_max = FfiAny {
            any_type: info.max_value.any_type,
            i64_value: info.max_value.i64_value,
            u64_value: info.max_value.u64_value,
            d_value: info.max_value.d_value,
            s_value: max_s_val,
        };
...
pub extern "C" fn emane_rs_config_free_manifest(manifest: FfiConfigManifest) {
    if manifest.data.is_null() || manifest.len == 0 {
        return;
    }
    ...
    for info in infos {
        unsafe {
            if !info.name.is_null() {
                let _ = std::ffi::CString::from_raw(info.name as *mut c_char);
            }
            if !info.usage.is_null() {
                let _ = std::ffi::CString::from_raw(info.usage as *mut c_char);
            }
            if !info.regex_pattern.is_null() {
                let _ = std::ffi::CString::from_raw(info.regex_pattern as *mut c_char);
            }
            if !info.min_value.s_value.is_null() {
                let _ = std::ffi::CString::from_raw(info.min_value.s_value as *mut c_char);
            }
            if !info.max_value.s_value.is_null() {
                let _ = std::ffi::CString::from_raw(info.max_value.s_value as *mut c_char);
            }

            if !info.values.data.is_null() && info.values.len > 0 {
                let vals = Vec::from_raw_parts(
                    info.values.data as *mut FfiAny,
                    info.values.len,
                    info.values.len,
                );
                for v in vals {
...
pub extern "C" fn emane_rs_config_free_update(update: FfiConfigUpdate) {
    if update.data.is_null() || update.len == 0 {
        return;
    }
    let items = unsafe {
        Vec::from_raw_parts(
            update.data as *mut FfiConfigItemUpdate,
            update.len,
            update.len,
        )
    };
    for item in items {
        unsafe {
            ...
            if !item.values.data.is_null() && item.values.len > 0 {
                let vals = Vec::from_raw_parts(
                    item.values.data as *mut FfiAny,
                    item.values.len,
                    item.values.len,
                );
                for v in vals {
...
fn parse_any(any_type: i32, s: &str) -> Option<FfiAnyOwned> {
    match any_type {
        ...
        1 | 3 | 5 | 7 => {
            ...
            s.parse::<u64>()
                .or_else(|_| s.parse::<f64>().map(|v| v as u64))
                .map(|v| FfiAnyOwned {
                    any_type,
                    i64_value: 0,
                    u64_value: v,
...
#[no_mangle]
pub extern "C" fn emane_rs_config_build_updates(
    build_id: u16,
    ...
    for req_item in slice {
        ...
        let info = match store.get(&name) {
            ...
            None => {
                write_error(
                    &format!("Parameter not registered {}", name),
                    err_buf,
                    err_len,
                );
                return FfiConfigUpdate {
        ...
        if req_item.values.len < info.min_occurs || req_item.values.len > info.max_occurs {
            write_error(
                &format!(
                    "Value occurrence out of range {} has {} values [{},{}]",
                    name, req_item.values.len, info.min_occurs, info.max_occurs
                ),
                err_buf,
                err_len,
            );
        ...
        if !req_item.values.data.is_null() && req_item.values.len > 0 {
            let str_slice =
                unsafe { std::slice::from_raw_parts(req_item.values.data, req_item.values.len) };
            for &c_str in str_slice {
                ...
                if !info.regex_ptr.0.is_null() {
                    ...
                    if !matched {
                        write_error(
                            &format!(
                                "Regular expression mismatch {} set to {} ({})",
                                name, val_str, info.regex_pattern
                            ),
                            err_buf,
                            err_len,
                        );
                ...
                let parsed = match parse_any(info.any_type, &val_str) {
                    ...
                    None => {
                        write_error(
                            &format!(
                                "Parameter value type incorrect {} (any_type={}, val='{}')",
                                name, info.any_type, val_str
                            ),
                            err_buf,
                            err_len,
                        );
                ...
                if info.has_min_max && info.any_type != 11 && info.any_type != 12 {
                    // not STRING or INET_ADDR
                    let mut out_of_range = false;
                    match info.any_type {
                        0 | 2 | 4 | 6 => {
                            // Signed: INT64, INT32, INT16, INT8
                            if parsed.i64_value < info.min_value.i64_value
                                || parsed.i64_value > info.max_value.i64_value
                            {
                                out_of_range = true;
                            }
                        }
                        1 | 3 | 5 | 7 => {
                            // Unsigned: UINT64, UINT32, UINT16, UINT8
                            if parsed.u64_value < info.min_value.u64_value
                                || parsed.u64_value > info.max_value.u64_value
                            {
                                out_of_range = true;
                            }
                        }
                    ...
                    if out_of_range {
                        write_error(
                            &format!(
                                "Out of range {} set to {}. Min: {}, Max: {}, parsed: {}",
                                name,
                                val_str,
                                info.min_value.u64_value,
                                info.max_value.u64_value,
                                parsed.u64_value
                            ),
                            err_buf,
                            err_len,
...
#[no_mangle]
pub extern "C" fn emane_rs_config_update(
    build_id: u16,
    updates: FfiConfigUpdate,
    err_buf: *mut c_char,
    err_len: usize,
) -> bool {
    ...
    let store = match s.stores.get_mut(&build_id) {
        ...
        None => {
            write_error(
                &format!("No component registered with build id {}", build_id),
                err_buf,
                err_len,
            );
            return false;
...
#[no_mangle]
pub extern "C" fn emane_rs_config_register_validator(
    build_id: u16,
...
mod tests {
    ...
    fn test_configuration_service() {
        let build_id: u16 = 1234;
        let mut err_buf = vec![0i8; 256];

        let name = CString::new("txpower").unwrap();
        let usage = CString::new("Transmit power").unwrap();
        let regex = CString::new("").unwrap();
        ...
        // Register a numeric item
        emane_rs_config_register_numeric_any(
            build_id,
            name.as_ptr(),
            8,
            1,
        ...
        // Query to fetch the manifest
        let manifest = emane_rs_config_get_manifest(build_id);
        assert_eq!(manifest.len, 1);

        let info_slice = unsafe { std::slice::from_raw_parts(manifest.data, manifest.len) };
        let fetched_name =
            unsafe { std::ffi::CStr::from_ptr(info_slice[0].name).to_string_lossy() };
        assert_eq!(fetched_name, "txpower");

        emane_rs_config_free_manifest(manifest);

        // Update the item
        let update_val_name = CString::new("txpower").unwrap();
        let update_val = FfiAny {
            any_type: 8,
            i64_value: 0,
            u64_value: 0,
            d_value: 50.0,
        ...
        let update_item = FfiConfigItemUpdate {
            name: update_val_name.as_ptr(),
            values: FfiAnyArray {
                data: &update_val,
                len: 1,
            },
        };
        let updates = FfiConfigUpdate {
            data: &update_item,
            len: 1,
        };

        let mut update_err_buf = vec![0i8; 256];
        let result = emane_rs_config_update(
            build_id,
            updates,
            update_err_buf.as_mut_ptr(),
            update_err_buf.len(),
        );
        assert!(result);

        // Query to check if the value was updated
        let query_names = FfiStringArray {
            data: ptr::null(),
            len: 0,
        };
        let query_result = emane_rs_config_query(build_id, query_names);
        assert_eq!(query_result.len, 1);

        let query_slice =
            unsafe { std::slice::from_raw_parts(query_result.data, query_result.len) };
        assert_eq!(query_slice[0].values.len, 1);

        let val_slice = unsafe {
            std::slice::from_raw_parts(query_slice[0].values.data, query_slice[0].values.len)
        };
        assert_eq!(val_slice[0].d_value, 50.0);

        emane_rs_config_free_update(query_result);
    }
}

...
use std::ffi::CStr;
use libc::{c_uint, setsockopt, SOL_SOCKET, SO_BINDTODEVICE, c_ushort};
...
extern "C" {
    fn emane_c_ota_manager_update_stat(
        uuid_ptr: *const u8,
        src_nem: u16,
        stat_type: u32,
    );
...
pub extern "C" fn emane_rs_ota_manager_open(
    ...
    let group_addr_c = unsafe { CStr::from_ptr(group_addr_str) }.to_string_lossy().into_owned();
    let device_c = unsafe { CStr::from_ptr(device_str) }.to_string_lossy().into_owned();
    ...
    let (ip, port) = {
        ...
        (parts[0].parse::<Ipv4Addr>().unwrap(), parts[1].parse::<u16>().unwrap())
    };
    ...
    if ip.is_multicast() {
        socket.join_multicast_v4(&ip, &Ipv4Addr::UNSPECIFIED).unwrap();
    ...
    loop {
        match socket.recv_from(&mut buf) {
            Ok((len, _)) => {
                    ...
                    if len >= 2 + header_len + std::mem::size_of::<PartInfo>() {
                        if let Ok(ota_header) = ota::OtaHeader::decode(&buf[2..2 + header_len]) {
                            ...
                            let part_info = unsafe { 
                                std::ptr::read_unaligned(buf.as_ptr().add(part_info_offset) as *const PartInfo) 
                            };
                            ...
                            if remote_uuid != local_uuid {
                                ...
                                let payload_index = part_info_offset + std::mem::size_of::<PartInfo>();
                                
                                if len == payload_index + u32_size {
                                    if u8_more == 0 && u32_offset == 0 {
                                        // Single part message
                                        let payload_info = ota_header.payload_info.unwrap_or_default();
                                        handle_ota_message(
                                        ...
                                        let entry = manager.part_store.entry(key).or_insert_with(|| {
                                            PartsData {
                                                total_size: 0,
                                        ...
                                        if entry.events_size == 0 && entry.controls_size == 0 && entry.data_size == 0 {
                                        ...
                                        entry.parts.insert(u32_offset, buf[payload_index..payload_index + u32_size].to_vec());
                                        entry.last_part_time = now;
                                        
                                        let expected_total = entry.events_size + entry.controls_size + entry.data_size;
                                        if entry.total_size == expected_total {
                                            // All parts received!
                                            let mut full_payload = Vec::with_capacity(expected_total);
                ...
                // Cleanup old parts
                let mut manager = get_ota_manager().lock().unwrap();
                if now.duration_since(manager.last_part_check_time).unwrap_or(Duration::ZERO) >= manager.part_check_threshold {
                    let threshold = manager.part_timeout_threshold;
                    manager.part_store.retain(|key, value| {
                        if now.duration_since(value.last_part_time).unwrap_or(Duration::ZERO) >= threshold {
                            // Update statistic upstream drop missing parts (Type = 3)
                            unsafe {
                                emane_c_ota_manager_update_stat(
                                    value.uuid.as_ptr(),
                                    key.source,
                                    3
                                );
...
pub extern "C" fn emane_rs_ota_manager_send_ota_packet(
    ...
    let events_slice = unsafe { std::slice::from_raw_parts(events_data, events_len) };
    let controls_slice = unsafe { std::slice::from_raw_parts(controls_data, controls_len) };
    let packet_slice = unsafe { std::slice::from_raw_parts(packet_data, packet_len) };
    
    while sent_bytes < total_payload_size {
        let total_wire_size = total_payload_size - sent_bytes + header_len + 2 + std::mem::size_of::<PartInfo>();
        ...
        let mut message = Vec::with_capacity(2 + header_len + std::mem::size_of::<PartInfo>() + payload_size);
        ...
        if p_offset >= c_start && p_offset < c_start + controls_len && p_size > 0 {
            ...
            message.extend_from_slice(&controls_slice[offset_in_controls..offset_in_controls + take]);
            p_size -= take;
            p_offset += take;
        }

...
pub extern "C" fn emane_rs_location_manager_destroy(ptr: *mut c_void) {
    if !ptr.is_null() {
        ...
        if !mgr.local_pov.is_null() {
            unsafe {
                emane_rs_ffi_free_pov(mgr.local_pov);
            }
        }
        for (_, pov) in mgr.store.drain() {
            unsafe {
                emane_rs_ffi_free_pov(pov);
            }
        }
        for (_, loc_info) in mgr.cache.drain() {
            unsafe {
                emane_rs_ffi_free_loc_info(loc_info);
            }
        }
    }
}
...
#[no_mangle]
pub extern "C" fn emane_rs_location_manager_insert_pov(
    ptr: *mut c_void,
    nem_id: u16,
    pov: *mut c_void,
) {
...
#[no_mangle]
pub extern "C" fn emane_rs_location_manager_get_cache(
    ptr: *mut c_void,
    nem_id: u16,
) -> *mut c_void {
    let mgr = unsafe { &mut *(ptr as *mut LocationManagerImpl) };
...
#[no_mangle]
pub extern "C" fn emane_rs_location_manager_set_cache(
    ptr: *mut c_void,
    nem_id: u16,
    loc_info: *mut c_void,
) {
    ...
    if let Some(old) = mgr.cache.insert(nem_id, loc_info) {
        if old != loc_info {
            unsafe {
                emane_rs_ffi_free_loc_info(old);
            }
        }
    }
}

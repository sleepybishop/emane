...
fn get_logger() -> &'static Mutex<LoggerState> {
    ...
    LOGGER.get_or_init(|| Mutex::new(LoggerState {
...
pub extern "C" fn emane_rs_log(level: i32, msg: *const c_char) {
    if !emane_rs_log_is_allowed(level) { return; }
    ...
    let lvl_str = if level >= 0 && level <= 4 { LEVEL_STRINGS[level as usize] } else { "?" };
    
    let formatted = format!("{:02}:{:02}:{:02}.{:06} {:5} {}\n",

...
extern "C" {
    ...
    pub fn emane_bentpipe_upstreampacket_stripLengthPrefixFraming(
    ...
    // MAC model functions
    pub fn emane_bentpipe_radiomodel_drop_registration_id(
        radiomodel: *mut c_void,
        src: u16,
        dst: u16,
        length: usize,
    );
    pub fn emane_bentpipe_radiomodel_drop_rx_off(
        radiomodel: *mut c_void,
        src: u16,
        pkt_data: *const c_void,
        pkt_len: usize,
    );

    pub fn emane_bentpipe_radiomodel_find_transponder(
        radiomodel: *mut c_void,
        msgs: *const ControlMessages,
        out_antenna_index: *mut u16,
        out_freq_hz: *mut u64,
        out_span: *mut u64,
        out_start_of_reception: *mut u64,
    ) -> i32;

    pub fn emane_bentpipe_radiomodel_enqueue_receive(
        radiomodel: *mut c_void,
        transponder_index: i32,
        pkt: *mut UpstreamPacket,
        msgs: *const ControlMessages,
        seq_num: u64,
    );

    pub fn emane_bentpipe_radiomodel_get_tos_to_transponder(
        radiomodel: *mut c_void,
        tos: u8,
    ) -> i32;
    pub fn emane_bentpipe_radiomodel_queue_manager_enqueue(
        radiomodel: *mut c_void,
        transponder_index: i32,
